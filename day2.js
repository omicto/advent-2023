...
const formatRow = (row) => {
  const [gameStr, gamesStr] = row.split(":");
  const id = Number.parseInt(gameStr.replace("Game ", ""));
  const games = gamesStr.split(";").map(getCubes);
  return { id, games };
};

const getCubes = (game) => {
  const regex = /(?<number>\d+)\s+(?<color>red|blue|green)/g;
  const result = { blue: 0, red: 0, green: 0 };
  let match;

  while ((match = regex.exec(game)) !== null) {
    let number = parseInt(match.groups.number, 10);
...
const isGamePossible = ({ red, green, blue }, query) => {
  return red <= query.red && green <= query.green && blue <= query.blue;
};
...
const solution1 = (input) => {
  ...
  const result = input
    .map(formatRow)
    .filter((game) => game.games.every(g => isGamePossible(g, query)))
    .map((game) => game.id)
    .reduce(sum);
  return result;
};
...
const solution2 = (input) => {
  const result = input
    .map(formatRow)
    .map((game) => {
      return  game.games.reduce(
        (acc, g) => {
          acc.red.push(g.red);
          acc.green.push(g.green);
          acc.blue.push(g.blue);
          return acc;
        },
        { red: [], green: [], blue: [] }
      );
    ...
    .map(
      ({ red, green, blue }) =>
        Math.max(...red) * Math.max(...blue) * Math.max(...green)
    )
    .reduce(sum);
  return result;
};
